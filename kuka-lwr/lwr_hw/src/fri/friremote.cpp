...
friRemote::friRemote(int port,char *hintToRemoteHost):remote(port,hintToRemoteHost)
{
  ...
  ///.d.r
  memset((void*)(& msr_copy),0x0,sizeof(msr_copy));
  memset((void*)(& cmd_copy), 0x0,sizeof(cmd_copy));
  {
...
{
  int rc=remote.Recv(&msr_copy);
  ///d.r to protect measured data
  {
    std::lock_guard<std::mutex> lock(msr_mutex_);
    memcpy(&msr, &msr_copy, sizeof(msr_copy));
  }
  return rc;
}
...
int friRemote::doSendData()
{
  ...
  seqCount++;
  ///d.r mutex to protect command data
  cmd_mutex_.lock();
...

{
  ...
  ///////d.r. commented cmd.cmd.cmdFlags=0;//////////
  //cmd.cmd.cmdFlags=0;
  //////////////////////////////////////////////////
  cmd_mutex_.lock();
  if (newJntPosition)
