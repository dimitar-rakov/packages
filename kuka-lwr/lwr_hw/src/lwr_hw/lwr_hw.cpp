...
void LWRHW::create(std::string name, std::string urdf_string)
{
    ...
    joint_msr_stiffness_.resize(n_joints_);
    joint_msr_damping_.resize(n_joints_);
...
void LWRHW::reset()
{
    for (int j = 0; j < n_joints_; ++j)
    {
        ...
        joint_msr_stiffness_[j] = 0.0;
        joint_msr_damping_[j] = 0.0;
                               ...
                               std::vector<transmission_interface::TransmissionInfo> transmissions)
{
    ...
    for(int j=0; j < n_joints_; j++)
    {
        ...
        joint_state_handle_.registerHandle(hardware_interface::JointStateHandle(
                                               joint_names_[j],
        ...
        hardware_interface::JointHandle joint_handle_position = hardware_interface::JointHandle(
                    joint_state_handle_.getHandle(joint_names_[j]),
        ...
        registerJointLimits(
              joint_names_[j], joint_handle_position, joint_handle_effort, joint_handle_velocity,
              urdf_model, &joint_lower_limits_[j], &joint_upper_limits_[j], &joint_velocity_limits_[j],
              &joint_acceleration_limits_[j], &joint_effort_limits_[j], &joint_jerk_limits_[j]);
    }

    // Register interfaces
    registerInterface(&joint_state_handle_);
...
void LWRHW::registerJointLimits(const std::string& joint_name,
                                ...
                                double *const acceleration_limit, double * const jerk_limit)
{
    ...
    *jerk_limit = std::numeric_limits<double>::max();
    ...
    // ToDo add acceleration and jerk limit
...
bool LWRHW::initKDLdescription(const urdf::Model *const urdf_model)
{
    ...
    if( root_name.empty() )
        tip_name = robot_namespace_ + std::string("_a6_link"); ; // default
    ...
    if (   ros::param::get(std::string("/") + robot_namespace_ + std::string("/gravity_wrt_robot_base/x"), gravity_(0))
           && ros::param::get(std::string("/") + robot_namespace_ + std::string("/gravity_wrt_robot_base/y"), gravity_(1))
           && ros::param::get(std::string("/") + robot_namespace_ + std::string("/gravity_wrt_robot_base/z"), gravity_(2))){
        ROS_INFO("Gravity vector w.r.t robot base is: x:%f  y:%f  z:%f", gravity_(0), gravity_(1), gravity_(2));
    }
    else{
...
bool LWRHW::canSwitch(const std::list<hardware_interface::ControllerInfo> &start_list, const std::list<hardware_interface::ControllerInfo> &stop_list) const
{
    ...
    for ( std::list<hardware_interface::ControllerInfo>::const_iterator it = start_list.begin(); it != start_list.end(); ++it )
    {
        ...
        else if( it->hardware_interface.compare( std::string("hardware_interface::EffortJointInterface") ) == 0 )
        {
            ...
            desired_strategies.push_back( JOINT_EFFORT );
