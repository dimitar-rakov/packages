...
bool LWRHWGazebo::init(ros::NodeHandle &nh)
{
  ...
  if( !(parent_set_) )
  {
    std::cout << "Did you forget to set the parent model? You must do that before init(). Exiting...\n";
    return false;
  }
  ...
  switch (getControlStrategy())
  {
    ...
    case JOINT_POSITION:
      ...
      for(size_t j=0; j < n_joints_; j++)
      {
        ...
        // d.r. workaround to fix ofsets caused by physical properties of the gazebo model
