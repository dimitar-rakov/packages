...
bool ImbvsWithObstaclesAvoidance::init(ros::NodeHandle &n)
{
  ...
  cb_obstacles_obj_status_ = -1;
  cb_robot_obj_status_ = -1;
  cb_joints_state_status_ = -1;
  cb_msr_features_status_ = -1;
  cb_des_features_status_ = -1;

  in_obstacles_obj_status_ = -1;
  in_robot_obj_status_ = -1;
  in_joints_state_status_ = -1;
  in_msr_features_status_ = -1;
  in_des_features_status_ = -1;
  ...
  cb_markers_obstacles_objects_ptr_.reset (new visualization_msgs::MarkerArray());
  in_markers_obstacles_objects_ptr_.reset (new visualization_msgs::MarkerArray());
  cb_markers_robot_objects_ptr_.reset (new visualization_msgs::MarkerArray());
  in_markers_robot_objects_ptr_.reset (new visualization_msgs::MarkerArray());
  cb_vis_features_ptr_.reset (new imbvs_with_obstacles_avoidance::VisFeature());
  in_vis_features_ptr_.reset (new imbvs_with_obstacles_avoidance::VisFeature());
  cb_joints_states_ptr_.reset (new sensor_msgs::JointState());
  in_joints_states_ptr_.reset (new sensor_msgs::JointState());
  traj_msg_ptr_.reset(new trajectory_msgs::JointTrajectory());
  all_data_msg_ptr_.reset( new std_msgs::Float64MultiArray ());
  ...
  joint_rep_field_ = Eigen::VectorXd::Zero(joint_number_);
  joint_rep_field_prev_ = Eigen::VectorXd::Zero(joint_number_);

  sub_visual_features_data_ = nh_.subscribe(features_data_topic_, 1, &ImbvsWithObstaclesAvoidance::featureExtractorCB, this);
  sub_joint_state_= nh_.subscribe(joint_states_topic_, 1, &ImbvsWithObstaclesAvoidance::jointsStatesCB, this);
  ...
  pub_all_data_ = nh_.advertise<std_msgs::Float64MultiArray>(nh_.getNamespace()+"/all_data", 1);
  pub_joint_traj_ = nh_.advertise<trajectory_msgs::JointTrajectory>( robot_command_topic_, 1 );
  pub_rep_vec_marker_ = nh_.advertise<visualization_msgs::Marker>(nh_.getNamespace()+"/repulsive_vector", 1 );
...
void ImbvsWithObstaclesAvoidance::starting(const ros::Time& time) {
  ...
  in_joints_state_status_ = 2;
}

void ImbvsWithObstaclesAvoidance::update(const ros::Time& time, const ros::Duration& period)
{

  // Obstacles objects mutex
  {
    std::lock_guard<std::mutex> guard(obst_mutex_cb_);
    in_markers_obstacles_objects_ptr_= cb_markers_obstacles_objects_ptr_;
    in_obstacles_obj_status_= cb_obstacles_obj_status_;
    // Safety timer
    if ((ros::Time::now()- safety_ton_obstacles_obj_).toSec()< 2.0 && in_obstacles_obj_status_> -1){ in_obstacles_obj_status_= 1; }
    else if ((ros::Time::now()- safety_ton_obstacles_obj_).toSec()> 2.0 && in_obstacles_obj_status_> -1){ in_obstacles_obj_status_= 0; }
    if (in_obstacles_obj_status_ == 0) ROS_WARN("Obstacle objects's topic is not longer available");
    else if (in_obstacles_obj_status_ == -1  && enb_obstacle_avoidance_) ROS_WARN_THROTTLE(5, "Waiting for obstacle objects's topic");
  }

  // Robot objects mutex
  {
    std::lock_guard <std::mutex>guard(robot_obj_mutex_cb_);
    in_markers_robot_objects_ptr_ = cb_markers_robot_objects_ptr_;
    in_robot_obj_status_= cb_robot_obj_status_;
    // Safety timer
    if ((ros::Time::now()- safety_ton_robot_obj_).toSec()< 2.0 && in_robot_obj_status_> -1){ in_robot_obj_status_= 1; }
    else if ((ros::Time::now()- safety_ton_robot_obj_).toSec()> 2.0 && in_robot_obj_status_> -1){ in_robot_obj_status_= 0; }
    if (in_robot_obj_status_ == 0) ROS_WARN("Robot objects's topic is not longer available");
    else if (in_robot_obj_status_ == -1 && enb_obstacle_avoidance_) ROS_WARN_THROTTLE(5, "Waiting for robot objects's topic");

    // Resize once
    if (Pb_os_min_.size()==0){
      Pb_os_min_.resize(in_markers_robot_objects_ptr_->markers.size());
      Pb_rs_min_.resize(in_markers_robot_objects_ptr_->markers.size());
    }
  }

  // Visual features mutex
  {
    std::lock_guard <std::mutex> guard (features_mutex_);
    in_vis_features_ptr_ = cb_vis_features_ptr_;

    // Exponential smoothing filter. Values closer to 0 weight the last smoothed value more heavily
    if (in_vis_features_ptr_->is_valid_msr_feature){
      for (size_t i = 0; i< in_vis_features_ptr_->size_s_msr; i++)
          s_msr_(i) = filters::exponentialSmoothing(in_vis_features_ptr_->s_msr[i], s_msr_(i), 0.25);
      for (size_t r = 0; r< in_vis_features_ptr_->rowsLmsr; r++){
        for (size_t c = 0; c< in_vis_features_ptr_->colsLmsr; c++)
          Lhat_msr_(r, c) = in_vis_features_ptr_->L_data_msr[r*in_vis_features_ptr_->colsLmsr + c];
      }
    }

    if (in_vis_features_ptr_->is_valid_des_feature){
      for (size_t i = 0; i< in_vis_features_ptr_->size_s_des; i++)
        s_des_(i) = in_vis_features_ptr_->s_des[i];

      for (size_t r = 0; r< in_vis_features_ptr_->rowsLmsr; r++){
        for (size_t c = 0; c< in_vis_features_ptr_->colsLmsr; c++){
          if (in_vis_features_ptr_->is_valid_des_feature)
            Lhat_des_(r, c) = in_vis_features_ptr_->L_data_des[r*in_vis_features_ptr_->colsLdes + c];
        }
      }
    }
    in_msr_features_status_ = cb_msr_features_status_;
    if ((ros::Time::now()- safety_ton_msr_features_).toSec()> 2.0 && in_msr_features_status_> -1){ in_msr_features_status_= 0; }
    if (in_msr_features_status_ == 0) ROS_WARN("Measured features's topic is not longer available");
    else if (in_msr_features_status_ == -1) ROS_WARN_THROTTLE(5, "Waiting for measured features's topic");
    else if (in_msr_features_status_ == 1) ROS_WARN_THROTTLE(2, "Measured features are out of FOV");

    in_des_features_status_ = cb_des_features_status_;
    if ((ros::Time::now()- safety_ton_des_features_).toSec()> 2.0 && in_des_features_status_> -1){ in_des_features_status_= 0; }
    if (in_des_features_status_ == 0) ROS_WARN("Desired features's topic is not longer available");
    else if (in_des_features_status_ == -1) ROS_WARN_THROTTLE(5, "Waiting for desired features's topic");
    else if (in_des_features_status_ == 1) ROS_WARN_THROTTLE(2, "Desired features are out of FOV");
  }

  // Joint states mutex
  {
  std::lock_guard <std::mutex> guard(joint_mutex_cb_);
  in_joints_states_ptr_= cb_joints_states_ptr_;
  in_joints_state_status_ =cb_joints_state_status_;
  // Safety timer
  if ((ros::Time::now()- safety_ton_joints_state).toSec()< 2.0 && in_joints_state_status_> -1){ in_joints_state_status_= 1; }
  else if ((ros::Time::now()- safety_ton_joints_state).toSec()> 2.0 && in_joints_state_status_> -1){ in_joints_state_status_= 0; }
  if (in_joints_state_status_ == 0) ROS_WARN("Joint states topic is not longer available");
  else if (in_joints_state_status_ == -1) ROS_WARN_THROTTLE(5, "Waiting for joint states topic");
  }

  //Update joint states
  if (in_joints_state_status_ >= 1) {
  ...
  for (unsigned int i = 0; i < traj_msg_ptr_->joint_names.size(); i++){
    for (unsigned int j = 0; j < in_joints_states_ptr_->name.size(); j++){
      if ( traj_msg_ptr_->joint_names[i] == in_joints_states_ptr_->name[j]){
        joint_msr_states_.q(i) =  in_joints_states_ptr_->position[j];
        joint_msr_states_.qdot(i) =  in_joints_states_ptr_->velocity[j];
  ...
  else if (in_joints_state_status_ == 1)
  ...
  if (in_joints_state_status_ == 2 && getTFs())
  {
    ...
    */
    ...
    // Safety common part
    if (enb_obstacle_avoidance_ && in_obstacles_obj_status_ == 1 && in_robot_obj_status_ == 1){
    ...
    if (in_msr_features_status_ ==2 && in_des_features_status_ ==2 && cmd_flag_ ==1 && !test_only_obst_avoidance_ && !restoring_initial_)
    {
      ...
      if (enb_obstacle_avoidance_ && in_obstacles_obj_status_ == 1 && in_robot_obj_status_ == 1){
        ...
        if ( !interaction_active_ && min_dist_ <= ro1_ && in_msr_features_status_ ==2 && in_des_features_status_ ==2){
    ...
    // Go to the position when the obstacle interaction first time occurs if it is free
    else if ((in_msr_features_status_ ==1 && in_des_features_status_ ==2 && cmd_flag_ ==1 && !test_only_obst_avoidance_) || restoring_initial_){
    ...
    traj_msg_ptr_->points.clear();
    traj_msg_ptr_->points.push_back(traj_points_);
  }
}

void ImbvsWithObstaclesAvoidance::publish(){
  if (!traj_msg_ptr_->points.empty()){
    if (USE_TRAJ_CLIENT){
      control_msgs::FollowJointTrajectoryGoal goal;
      goal.trajectory = *traj_msg_ptr_;
      //goal.trajectory .header.stamp = ros::Time::now();
      traj_client_ptr_->sendGoal(goal);
    }
    else
      pub_joint_traj_.publish(traj_msg_ptr_);
  }
  if (!all_data_msg_ptr_->data.empty())
    pub_all_data_.publish (all_data_msg_ptr_);
}

bool ImbvsWithObstaclesAvoidance::getParameters(){
   urdf::Model model;
  ...
  if (!nh_.getParam("root_name", root_name_)){
    ROS_ERROR_STREAM("ImbvsWithObstaclesAvoidance: No root name found on parameter server ("<<nh_.getNamespace()<<"/root_name)");
    return false;
  }

  if (!nh_.getParam("tip_name", tip_name_)){
    ROS_ERROR_STREAM("ImbvsWithObstaclesAvoidance: No tip name found on parameter server ("<<nh_.getNamespace()<<"/tip_name)");
    return false;
  }
  ...
  for (unsigned int i = 0; i < joint_number_ && link; i++)
  {
    ...
    traj_msg_ptr_->joint_names.insert(traj_msg_ptr_->joint_names.begin(),joint->name);
...
void ImbvsWithObstaclesAvoidance::featureExtractorCB(const imbvs_with_obstacles_avoidance::VisFeature::ConstPtr &msg){

  std::lock_guard<std::mutex> guard(features_mutex_);
  cb_vis_features_ptr_ = msg;
  // First time received data initializes the size of the used matrices and the gain
  if (cb_msr_features_status_ ==-1 || s_msr_.size() < cb_vis_features_ptr_->size_s_msr)
  {
    for (size_t i = 0; i< in_vis_features_ptr_->size_s_msr; i++){
      s_msr_(i) = in_vis_features_ptr_->s_msr[i];
      s_cmd_obs_dot_(i)  = 0.0;
    }
    Lhat_msr_ = MatrixXd::Zero(in_vis_features_ptr_->rowsLmsr, in_vis_features_ptr_->colsLmsr);
    ROS_INFO("First msr features data is received");
  }

  // for initialization at start set des = msr otherwise is overlapped
  if (cb_des_features_status_ == -1  || s_des_.size() < cb_vis_features_ptr_->size_s_msr ) {
    s_des_ = s_msr_;
    Lhat_des_ = Lhat_msr_;
  }
  cb_msr_features_status_ = (msg->is_valid_msr_feature)? 2 : 1;
  safety_ton_msr_features_ = ros::Time::now();
  cb_des_features_status_ = (msg->is_valid_des_feature)? 2 : 1;
  safety_ton_des_features_ = ros::Time::now();

}

void ImbvsWithObstaclesAvoidance::obstaclesObjectsCB(const visualization_msgs::MarkerArray::ConstPtr &msg){
  std::lock_guard<std::mutex> guard(obst_mutex_cb_);
  cb_markers_obstacles_objects_ptr_ = msg;
  cb_obstacles_obj_status_ = 1;
  safety_ton_obstacles_obj_ = ros::Time::now();
}

void ImbvsWithObstaclesAvoidance::robotObjectsCB(const visualization_msgs::MarkerArray::ConstPtr &msg){
  std::lock_guard<std::mutex> guard(robot_obj_mutex_cb_);
  cb_markers_robot_objects_ptr_ = msg;
  cb_robot_obj_status_ = 1;
...
void ImbvsWithObstaclesAvoidance::recordAllData(){
  ...
  // All data for rosbag
  all_data_msg_ptr_->data.clear();
  //measured angles 3-9
  for (unsigned int i = 0; i < joint_number_; i++)
    all_data_msg_ptr_->data.push_back(joint_msr_states_.q(i));

  //desired angles 10-16
  for (unsigned int i = 0; i < joint_number_; i++)
    all_data_msg_ptr_->data.push_back(joint_des_states_.q(i));

  //measured velocity 17-23
  for (unsigned int i = 0; i < joint_number_; i++)
    all_data_msg_ptr_->data.push_back(joint_msr_states_.qdot(i));

  //desired velocity 24-30
  for (unsigned int i = 0; i < joint_number_; i++)
    all_data_msg_ptr_->data.push_back(joint_des_states_.qdot(i));

  //repulsive field velocity(simple 31-37 )
  for (unsigned int i = 0; i < joint_rep_field_.size(); i++)
    all_data_msg_ptr_->data.push_back(joint_rep_field_(i));


  // Fbe_ 38-43
  double roll, pitch, yaw;
  KDL::Frame Fwc = Fwb_ * Fbe_ * Fec_;
  all_data_msg_ptr_->data.push_back(Fwc.p.x());
  all_data_msg_ptr_->data.push_back(Fwc.p.y());
  all_data_msg_ptr_->data.push_back(Fwc.p.z());
  Fwc.M.GetRPY(roll, pitch, yaw);
  all_data_msg_ptr_->data.push_back(roll);
  all_data_msg_ptr_->data.push_back(pitch);
  all_data_msg_ptr_->data.push_back(yaw);

  if (in_msr_features_status_ ==2 && in_des_features_status_ ==2 && !test_only_obst_avoidance_ ){

    //measured features vector (simple 44-49 )
    for (unsigned int i = 0; i < s_msr_.size(); i++)
      all_data_msg_ptr_->data.push_back(s_msr_(i));

    //desired features vector (simple 50-55 )
    for (unsigned int i = 0; i < s_des_.size(); i++)
      all_data_msg_ptr_->data.push_back(s_des_(i));


    //error features vector_dot (simple 56-61 )
    for (unsigned int i = 0; i < s_cmd_dot_.size(); i++)
      all_data_msg_ptr_->data.push_back(s_cmd_dot_(i));
    ...
    for (long i = 0; i < (svd1.singularValues()).size(); i++)
      all_data_msg_ptr_->data.push_back((svd1.singularValues())(i));
    ...
    for (long i = 0; i < (svd2.singularValues()).size(); i++)
      all_data_msg_ptr_->data.push_back((svd2.singularValues())(i));
...
void ImbvsWithObstaclesAvoidance::jointsStatesCB(const sensor_msgs::JointState::ConstPtr &msg){
  std::lock_guard <std::mutex> guard(joint_mutex_cb_);
  cb_joints_states_ptr_= msg;
  cb_joints_state_status_ = (in_joints_state_status_ == 2)? 2 : 1;
  safety_ton_joints_state = ros::Time::now();
}
...
void ImbvsWithObstaclesAvoidance::calcObjectsFrames(std::vector< size_t > &link_to_base_indeces){
  ...
  for (size_t i = 0; i < in_markers_obstacles_objects_ptr_->markers.size(); i++){
    tf::poseMsgToKDL(in_markers_obstacles_objects_ptr_->markers[i].pose, Fwo);
  ...
  for(size_t i = 0; i< in_markers_obstacles_objects_ptr_->markers.size(); i++){
    for(size_t j = 0; j< tf_names_.size(); j++){
      if (tf_names_[j] == in_markers_obstacles_objects_ptr_->markers[i].header.frame_id){
        fk_pos_solver_->JntToCart(joint_msr_states_.q, Fbj, static_cast<int>(j));
        link_to_base_indeces.push_back(j);
        tf::poseMsgToKDL(in_markers_obstacles_objects_ptr_->markers[i].pose, Fjs);
...
void ImbvsWithObstaclesAvoidance::calcMinDistances(){
  ...
  for (size_t j = 0; j< Fbs_.size(); j++){
    ...
    geometry_msgs::Vector3 os_s = in_markers_obstacles_objects_ptr_->markers[j].scale;
    ...
    for (size_t i = 0; i< in_markers_obstacles_objects_ptr_->markers.size(); i++){
      ...
      geometry_msgs::Vector3 ro_s = in_markers_obstacles_objects_ptr_->markers[i].scale;
...
void ImbvsWithObstaclesAvoidance::obstaclesProcesing(double &dst_min_dist, double &dst_max_ni1){
  ...
  if (in_markers_obstacles_objects_ptr_->markers.size()){
