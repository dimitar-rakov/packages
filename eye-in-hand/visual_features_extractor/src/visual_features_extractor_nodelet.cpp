...
namespace visual_features_extractor
{
  ...
  class VisualFeaturesExtractorNodelet : public nodelet::Nodelet
  {
  public:
    VisualFeaturesExtractorNodelet(){}
    ~VisualFeaturesExtractorNodelet(){}
    ...
    virtual void onInit()
    {
      ...
      if (visual_features_extractor_ptr_->init(nh)){
        NODELET_INFO_STREAM("Nodelet initialised. [" << name << "]");
        run_thread_ptr_.reset(new boost::thread(boost::bind(&VisualFeaturesExtractorNodelet::run, this)));
      }
      else
    ...
    void run() {
      ...
      while(ros::ok())
      {
          ...
          visual_features_extractor_ptr_->update(ros::Time::now(), work_period);
  ...
  private:
    boost::shared_ptr<VisualFeaturesExtractor> visual_features_extractor_ptr_;
    boost::shared_ptr<boost::thread> run_thread_ptr_;
  };

  } // namespace examples

  PLUGINLIB_EXPORT_CLASS(visual_features_extractor::VisualFeaturesExtractorNodelet, nodelet::Nodelet);
